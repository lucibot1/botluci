...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5522988714803 (22) 988714803\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `ja tava demorando @${num.split('@')[0]}👋 nem vai fazer falta`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ carregando... se eu não enviar tente novamente em 15s⌛',
				success: '✔️ Sucesso ✔️',
				...
				only: {
					group: '[❗] Este comando só pode ser usado em grupos poha! ❌',
					ownerG: '[❗] Este comando só pode ser usado pelo grupo proprietário!(luci/nabuto😌) ❌',
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "luci", vcard: vcard}, MessageType.contact, { quoted: mek})
	...
	case 'hidetag':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`Insira numeros\nExemplo : ${prefix}spamcall 5522988714803...`)
	    ...
	    case 'kapankah':
			...
			case 'closegc':
	          ...
	          if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*Canção encontrada!!!*\nJudul : ${anu.result.title}\nFonte : ${anu.result.source}\nTamanho : ${anu.result.size}\n\n*ESPERE POHA TA ENVIANDO POR FAVOR, NÃO SPAM YA PAI*`
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply(mess.blank)
                                        if (!isUser) return reply(mess.only.daftarB)
					tels = body.slice(7)
					if (tels.length > 15) return reply('O texto é muito longo poha, até 20 caracteres')
					reply(mess.wait)
            ...
            break
				...
				case 'nsfwass':
					...
					case 'nsfwahegao':
						try {
							if (!isNsfw) return reply('❌ *NSFW Ativado* ❌')
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        ...
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *TO AFK CCT ${tels} NAUM PERTURBE TALKEI?*`,
